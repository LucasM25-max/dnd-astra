...
function weaponCard(id: string, group: 'primary' | 'offhand', selectedId: string | null): string {
  ...
  return `<button type="button" class="cc-weapon${isSel ? ' selected' : ''}" data-weapon-${group}="${id}" aria-pressed="${isSel}">`
    + `<span class="cc-weapon-glyph">${icon(w.icon, 'cc-icon-lg')}</span><span class="cc-weapon-body"><span class="cc-weapon-name">${esc(w.name)}</span>`
...
export function renderEquipmentLoadout(d: CharacterDraft): string {
  ...
  const fixedIcon = (w: ReturnType<typeof weaponDef>): string => (w ? icon(w.icon) : '');
  return `<div class="cc-loadout">`
    ...
    + `<li>${fixedIcon(bow)}<span><strong>${esc(bow?.name ?? 'Longbow')}</strong> ${fixedIcon(quiver)}<strong>Quiver</strong> — ${eq.arrows} arrows for answers at range.</span></li>`
