...
export function hpGlobeSVG(current: number, max: number, size = 64): string {
  ...
  const low = frac <= 0.25;
  // Unique gradient id per globe — several globes can share one document.
  const gid = `hpglobe-grad-${globeInstance++}`;
  return `<svg class="hp-globe${low ? ' low' : ''}" viewBox="0 0 64 64" width="${size}" height="${size}" role="img" aria-label="Health ${current} of ${max}">`
    ...
    + `</radialGradient></defs>`
    + `<circle cx="32" cy="32" r="29" fill="#102019" stroke="#d0b781" stroke-width="2.5"/>`
    + `<circle cx="32" cy="32" r="24" fill="#1c0e0c"/>`
    + `<circle cx="32" cy="32" r="24" fill="none" stroke="#33222a" stroke-width="7"/>`
    + `<circle cx="32" cy="32" r="24" fill="none" stroke="url(#${gid})" stroke-width="7" stroke-linecap="round"`
