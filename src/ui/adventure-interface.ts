...
export class AdventureInterface {
  ...
  private manifestHTML() {
    ...
    }).join('')}</div>${this.world.adventure.mounted ? `<button class="primary-action" data-action="dismount" ${!store.arrived ? 'disabled' : ''}>${icon('log-out')} ${store.arrived ? 'Step down and tie off the oxen' : 'Cargo is accessible after the opening journey'} ${icon('arrow-right')}</button>` : `<div class="dialog-footnote">${icon('footprints')} ${this.world.getState().oxenTied ? 'The oxen are tied off. Follow the northwest trail on foot.' : 'The wagon is nearby. Walk alongside a container and press E to open it.'}</div>`}<div class="manifest-note">Cargo values are fixed sale appraisals. The wagon and oxen are not part of the 100 gp inventory.</div>`;
  }
