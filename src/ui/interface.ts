import { createIcons, ArrowRight, ArrowUp, ArrowUpRight, Camera, Check, ChevronRight, CircleHelp, Cloud, Compass, Download, Expand, Eye, Footprints, Headphones, Leaf, Maximize, Moon, Mouse, PersonStanding, Play, RotateCcw, SlidersHorizontal, Sun, Volume2, VolumeX, X } from 'lucide';
...
type DialogKind = 'settings' | 'map' | 'help' | 'pause' | 'inspect';
interface Preferences { quality: Quality; atmosphere: Atmosphere; volume: number; sensitivity: number; invertY: boolean }
const iconSet = { ArrowRight, ArrowUp, ArrowUpRight, Camera, Check, ChevronRight, CircleHelp, Cloud, Compass, Download, Expand, Eye, Footprints, Headphones, Leaf, Maximize, Moon, Mouse, PersonStanding, Play, RotateCcw, SlidersHorizontal, Sun, Volume2, VolumeX, X };
...
export class WorldInterface {
  ...
  private bind() {
    ...
    click('#inspect-prompt', () => this.openDialog('inspect'));
    click('#touch-jump', () => { if (!this.world.controller.started) this.start(); this.world.controller.jump(); });
    ...
    $('#dialog').addEventListener('click', e => {
      ...
      if (action === 'reset') { this.world.controller.reset(); this.closeDialog(); this.toast('Back where your journey began.'); }
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.code === 'KeyE' && this.state.landmark === 'ambush' && this.state.started) this.openDialog('inspect');
    }, opts);
  ...
  start() {
    if (!this.world.controller.started) {
      this.world.controller.start(); document.body.dataset.playing = 'true';
  ...
  private update(state: WorldState) {
    ...
    $('#inspect-prompt').hidden = state.landmark !== 'ambush' || !state.started || !!this.dialog || this.photo;
  ...
  openDialog(kind: DialogKind) {
    this.dialog = kind; $('.hud').inert = true; this.world.controller.setPaused(true); this.audio.setPaused(true);
    const backdrop = $('#dialog-backdrop'), dialog = $('#dialog');
    dialog.dataset.kind = kind; dialog.innerHTML = this.dialogHTML(kind); backdrop.hidden = false; document.body.dataset.modal = 'true';
    refreshIcons();
    if (kind === 'map') requestAnimationFrame(() => this.map.render($('#world-map-canvas') as HTMLCanvasElement, this.world.getState(), true));
    requestAnimationFrame(() => { (dialog.querySelector<HTMLElement>('[data-action="close"]') ?? dialog).focus(); });
  }
  closeDialog() {
    this.dialog = null; $('.hud').inert = false; $('#dialog-backdrop').hidden = true; document.body.dataset.modal = 'false';
    this.world.controller.setPaused(false); this.audio.setPaused(false);
    this.world.renderer.domElement.focus({ preventScroll: true });
  }
  private dialogHTML(kind: DialogKind) {
    ...
    if (kind === 'help') return `${close}<div class="dialog-eyebrow">A FEW WAYS TO FIND YOUR FEET</div><h2 id="dialog-title">Take the scenic route.</h2><p class="dialog-description">No objectives. No rush. This is your place to explore.</p><div class="help-grid">${[
      ['W A S D', 'Walk', 'Arrow keys work, too.'], ['SHIFT', 'Sprint', 'Hold while moving.'], ['SPACE', 'Jump', 'A little spring in your step.'], ['MOUSE', 'Look around', 'Click to capture, or click and drag.'], ['V', 'Change perspective', 'First person or third person.'], ['SCROLL', 'Camera distance', 'Zoom in or out in third person.'], ['M', 'World map', 'Find your place in the woodland.'], ['P', 'Photo mode', 'Hide the interface. Keep the moment.'], ['E', 'Inspect', 'Look closer at the ambush clearing.'], ['ESC', 'Pause / release mouse', 'Take a breath. The world will wait.'],
    ...
    if (kind === 'pause') return `${close}<div class="pause-emblem">${icon('leaf')}</div><div class="dialog-eyebrow">THE ROAD CAN WAIT</div><h2 id="dialog-title">A moment of quiet.</h2><p class="dialog-description">Your little corner of the world will be right here.</p><div class="pause-actions"><button class="primary-action" data-action="resume">${icon('play')} Back to the woodland ${icon('arrow-right')}</button><button data-action="settings">${icon('sliders-horizontal')} World settings ${icon('chevron-right')}</button><button data-action="help">${icon('compass')} A guide to exploring ${icon('chevron-right')}</button><button data-action="reset">${icon('rotate-ccw')} Return to the starting point</button></div><div class="dialog-footnote">TRIBOAR TRAIL · THE SWORD COAST</div>`;
    return `${close}<div class="dialog-eyebrow">A STORY LEFT BEHIND</div><h2 id="dialog-title">An uneasy silence.</h2><div class="inspect-divider">◇</div><p class="inspect-copy">Two fallen horses lie across the old road. Their saddlebags have been emptied, and black-fletched arrows are scattered in the dust.</p><p class="inspect-copy">To the north, a narrow trail disappears between the trees. Bent grass and disturbed earth suggest someone passed this way.</p><div class="inspect-note">${icon('leaf')} An environmental scene only. There are no enemies or combat in this preview.</div><button class="primary-action" data-action="close">Leave it to the forest ${icon('arrow-right')}</button>`;
  }
  ...
  dispose() { this.abort.abort(); this.audio.dispose(); clearTimeout(this.toastTimer); clearTimeout(this.discoveryTimer); }
}
