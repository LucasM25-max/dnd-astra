/**
 * The Cragmaw ambush on the Triboar Trail - updated for new 10+10 mile main trail and 0.5 mile thin trail.
 * Main trail is 15ft wide, thin trail 5ft wide, both winding through 2 miles of forest with stream.
 */
import type { Vec2 } from './encounter';

/** Where the looted horses stand, in world metres - now east of spawn on main trail. */
export const AMBUSH_CENTRE: Vec2 = { x: 14, z: 0 };
/** Approaching within this many metres of the horses springs the trap. */
export const AMBUSH_TRIGGER_RADIUS = 5.0;
/** Mouth of the goblin trail; the last goblin runs for it - north of ambush, 0.5 mile thin trail start. */
export const GOBLIN_TRAIL_MOUTH: Vec2 = { x: 12, z: -6 };
...
/** Two goblins on either side of the road, as written - updated positions near new ambush. */
export const AMBUSHERS: AmbusherSpawn[] = [
  { id: 'goblin-north-1', monsterId: 'goblin', name: 'Goblin in the thicket', side: 'north', hide: { x: 12, z: -5.5 }, strike: { x: 13, z: -2.2 } },
  { id: 'goblin-north-2', monsterId: 'goblinArcher', name: 'Goblin archer', side: 'north', hide: { x: 16, z: -6.2 }, strike: { x: 15, z: -3.0 } },
  { id: 'goblin-south-1', monsterId: 'goblin', name: 'Goblin in the bracken', side: 'south', hide: { x: 12, z: 5.2 }, strike: { x: 13, z: 2.8 } },
  { id: 'goblin-south-2', monsterId: 'goblinArcher', name: 'Goblin archer', side: 'south', hide: { x: 17, z: 4.8 }, strike: { x: 16, z: 2.5 } },
];
...
export const SCATTER: ScatterProp[] = [
  {
    id: 'map-case', kind: 'mapCase', position: { x: 14.5, z: 0.8 }, rotation: 0.7,
  ...
  {
    id: 'saddlebag-near', kind: 'saddlebag', position: { x: 13.2, z: 0.2 }, rotation: -0.7,
  ...
  {
    id: 'saddlebag-far', kind: 'saddlebag', position: { x: 15.1, z: -0.4 }, rotation: 1.15,
  ...
  {
    id: 'fabric-1', kind: 'fabric', position: { x: 13.5, z: 1.4 }, rotation: 2.1,
  ...
  {
    id: 'fabric-2', kind: 'fabric', position: { x: 15.3, z: 1.1 }, rotation: -1.4,
  ...
  {
    id: 'oddment-1', kind: 'oddment', position: { x: 13.1, z: 1.2 }, rotation: 0.4,
  ...
  {
    id: 'oddment-2', kind: 'oddment', position: { x: 15.2, z: 0.3 }, rotation: -0.2,
...
export const SPENT_ARROWS: { position: Vec2; rotation: number; pitch: number }[] = [
  { position: { x: 13.6, z: -0.6 }, rotation: 0.62, pitch: 1.36 },
  { position: { x: 12.9, z: 1.1 }, rotation: -1.13, pitch: 1.48 },
  { position: { x: 14.8, z: 1.4 }, rotation: 2.31, pitch: 1.29 },
  { position: { x: 15.2, z: -0.2 }, rotation: -0.44, pitch: 1.51 },
  { position: { x: 14.0, z: -0.9 }, rotation: 1.72, pitch: 1.41 },
  { position: { x: 12.6, z: -0.4 }, rotation: 0.18, pitch: 1.33 },
  { position: { x: 15.6, z: 1.2 }, rotation: -2.02, pitch: 1.46 },
  { position: { x: 14.3, z: 2.0 }, rotation: 0.94, pitch: 1.38 },
];
...
export const TRAIL_DISCOVERY = {
  ...
  text: 'Behind the thickets on the north side of the road, the undergrowth has been beaten into a path. It runs north, away from the main trail, and it has been used often. This is the 5-foot Cragmaw trail, 0.5 miles to the hideout.',
  check: {
    ...
    failure: 'Something has been using this path regularly. Small overlapping tracks suggest many goblins, and two broad drag marks run north, but you cannot make out an exact count.',
  },
  distanceMiles: 0.5,
};

export const AFTERMATH_NOTE = 'Now that nothing is trying to kill you, the shape of the place is obvious. The thickets on both sides have been hollowed out from behind into shooting blinds. There are old arrow shafts trodden into the leaf litter, a cold fire pit behind the northern bank, and a heap of gnawed bones. This is not where an ambush happened. This is where ambushes happen. Beyond, the main trail winds east for 10 miles then turns south for another 10, with 2 miles of forest and a stream to the west.';

// Traps on the goblin trail - 0.5 mile trail with traps at 200m and 500m
...
export const TRAIL_TRAPS: TrailTrap[] = [
  {
    id: 'snare', name: 'Hidden snare', atMinutes: 3, atMetres: 200,
    spot: { skill: 'perception', dc: 15 }, save: { ability: 'dex', dc: 10 },
    damage: '1d6', damageType: 'bludgeoning',
    description: 'A weighted cord buried under the leaf litter on the 5-foot trail, tensioned against a bent sapling.',
  ...
  {
    id: 'pit', name: 'Camouflaged pit', atMinutes: 7, atMetres: 500,
    spot: { skill: 'perception', dc: 15 }, save: { ability: 'dex', dc: 10 },
    damage: '1d6', damageType: 'bludgeoning',
    description: 'Six feet across, ten feet deep, roofed with branches and a careful layer of leaves on the narrow trail.',
...
export const CAPTURE = {
  ...
  intimidateDc: 10,
  knowledge: [
    'The Cragmaws took the dwarf and his man alive and hauled them up the 0.5 mile trail north.',
    'The hideout is a cave mouth 0.5 miles along, behind a stream and a screen of thorns.',
    'There are two traps on the way: a snare first at 200m, then a pit at 500m. A goblin who wants to keep breathing will walk you round both.',
