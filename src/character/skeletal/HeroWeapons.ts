...
function buildQuiver(m: WeaponMats): THREE.Group {
  ...
  for (let i = 0; i < 6; i++) {
    const [ax, az] = ring[i];
    const top = heights[i];
    g.add(mesh(new THREE.CylinderGeometry(0.0045, 0.0045, 0.5, 6), m.wood, ax, top - 0.25, az));
    const fletch = mesh(new THREE.ConeGeometry(0.014, 0.07, 6), i % 2 ? m.fletch : m.string, ax, top + 0.035, az);
    fletch.scale.z = 0.45;
    g.add(fletch);
  }
  return g;
