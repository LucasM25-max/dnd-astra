...
import { journeyPose, JOURNEY_START_DISTANCE, JOURNEY_END_DISTANCE } from '../src/game/road';
import { pathAmount, roadLength } from '../src/engine/landscape';
...
describe('the Narrator and opening route', () => {
  ...
  it('keeps the opening wagon on the new 10+10 mile main trail and ends near ambush', () => {
    const start = journeyPose(0);
    const end = journeyPose(1);
    // Journey should be along main trail, near spawn, on road
    expect(pathAmount(start.x, start.z)).toBe(1);
    expect(pathAmount(end.x, end.z)).toBe(1);
    expect(Number.isFinite(start.yaw)).toBe(true);
    expect(Number.isFinite(end.yaw)).toBe(true);
    // Distance should be within road length
    expect(JOURNEY_START_DISTANCE).toBeGreaterThan(0);
    expect(JOURNEY_END_DISTANCE).toBeGreaterThan(JOURNEY_START_DISTANCE);
    expect(JOURNEY_END_DISTANCE).toBeLessThan(roadLength());
    // End should be near ambush (14,0) - within 50m on 15ft main trail
    expect(Math.hypot(end.x - 14, end.z - 0)).toBeLessThan(50);
    for (let i = 0; i <= 100; i++) {
      const p = journeyPose(i / 100);
      expect(pathAmount(p.x, p.z)).toBe(1);
